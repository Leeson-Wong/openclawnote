...
- **查询接口** - 提供接口查询所有可用模型列表
- **分类展示** - 按提供商分类展示模型
- **模型详情** - 显示模型的详细信息（名称、ID、上下文窗口、价格等）
- **过滤功能** - 支持按提供商、能力、上下文大小等条件过滤
- **搜索功能** - 支持按模型名称或描述搜索
- **缓存列表** - 缓存模型列表，减少 API 调用
- **刷新列表** - 手动刷新模型列表
- **显示配置状态** - 标记哪些模型已配置，哪些未配置
- **示例输出**：
  ```
  Available Models:
  ├─ Anthropic
  │  ├─ claude-3-5-sonnet-20241022 (Claude 3.5 Sonnet) [✓ Configured]
  │  ├─ claude-3-opus-20240229 (Claude 3 Opus) [✗ Not configured]
  │  └─ ...
  ├─ OpenAI
  │  ├─ gpt-4o (GPT-4o) [✓ Configured]
  │  └─ ...
  ```
...
- **会话级选择** - 可以为每个会话单独选择模型
- **命令级选择** - 在对话中可以临时切换模型
- **模型优先级** - 设置主模型和备用模型，主模型不可用时自动切换
- **智能选择** - 根据任务类型自动选择最合适的模型（如代码任务选 Codex）
- **记忆选择** - 记住用户的选择，下次自动使用
- **配置示例**：
  ```json
  {
...
#### 模型别名
- **创建别名** - 为模型创建易记的别名（如 "claude" 代替完整 ID）
- **系统别名** - 预定义常用模型的别名（如 "gpt4" → "gpt-4-turbo"）
- **用户别名** - 用户可以自定义别名
- **别名解析** - 使用别名时自动解析为真实的模型 ID
- **别名冲突** - 处理别名冲突，优先使用用户定义的别名
- **示例配置**：
  ```json
  {
    "models": {
...
#### 模型兼容性检查
- **能力检查** - 检查模型是否支持所需能力（如工具调用、多模态）
- **参数验证** - 验证模型支持的参数（如温度、Top P）
- **上下文验证** - 检查上下文长度是否超出模型限制
- **功能兼容** - 检查技能/工具是否与选定的模型兼容
- **警告提示** - 使用不兼容模型时给出警告
- **自动建议** - 根据需求推荐兼容的模型
- **兼容性矩阵** - 提供模型功能兼容性矩阵
...
#### 模型超时配置
- **默认超时** - 设置默认的模型请求超时时间
- **模型级超时** - 为不同模型设置不同的超时时间
- **任务级超时** - 根据任务类型调整超时（如长任务设置更长超时）
- **重试超时** - 配置重试的超时策略
- **超时处理** - 超时时自动重试或返回错误
- **配置示例**：
  ```json
  {
    "models": {
      "timeouts": {
        "default": 60000,
        "anthropic/claude-3-5-sonnet-20241022": 120000,
        "openai/gpt-4o": 90000
      }
    }
  }
...
#### 模型参数配置
- **通用参数** - 支持配置温度、最大 Token、Top P、频率惩罚等
- **模型特定参数** - 支持模型特有的参数（如 Claude 的 thinking 参数）
- **默认参数** - 设置默认的模型参数
- **场景预设** - 预设不同场景的参数（创意、精确、代码等）
- **动态调整** - 根据任务动态调整参数
- **参数模板** - 为不同任务创建参数模板
- **配置示例**：
  ```json
